 ...
 * @LastEditTime: 2022-05-16 19:58:29
 * @Description: 
 */
...
class PageBreaker {
  ...
  Future<List<YDPage>> splitPage() async {
    List<YDPage> results = [];
    //当前页面的文字
    ...
    while (currText.length > 0) {
      ...
      var titleOffset =
          titlePainter == null ? 0.0 : titlePainter.height + titleMargin;
      //计算内容
      ...
      while (true) {
        ...
        if (maybeHeight > drawSize.height - titleOffset) {
          //超过了限制高度
          maxIndex = maybeIndex;
        } else {
          //没超过
      ...
      var textPainter = TextPainter(
        text: TextSpan(text: currText, style: contentString.style),
        textDirection: TextDirection.ltr,
      );
